...
{
public:
	Worm(SDL_Renderer* renderer, const char* path, int x, int y, int w, int h, float angle, string tag, bool hasAnimations, int range, float scale) : Enemy(renderer, path, x, y, w, h, angle, tag, hasAnimations, range, scale)
	{
	...
	void Update(SDL_Renderer* renderer) override
	{
		...
		{
			Game::gameObjects.push_back(new StraightProjectile(renderer, "assets/monsters/worm/fireBall.png", position.x + (wormSpriteOffsetX * scale * (flipStatus == SDL_FLIP_NONE ? scale : 1)), position.y + (wormSpriteOffsetY * scale) + 15, 32, 32, 0.0f, "Projectile", false, 5, (flipStatus == SDL_FLIP_NONE ? 1 : -1), 1, true));
